...
## Постоянная задача — бюджет токенов и промпта (сквозная, не закрывается)

Не спринт, а всегда-действующее ограничение: держать промпт коротким.

**Почему.** Машина владельца CPU-only — длина промпта главный источник латентности
(docs/01 §4.2). Часть промпта неизменна между запросами и кэшируется Ollama
(KV-cache префикса), но «горячий хвост» — тематические нуджи, схемы инструментов,
свежая история — пересчитывается на **каждый** запрос. Его режем в первую очередь.

**Замеры (2026-07-26, пример «поставь задачу купить молоко завтра в 9»):**
`system.md` ~1950 токенов (стабильный префикс, в основном кэшируется),
`TASK_NUDGE` ~340, схемы 9 инструментов ~500, весь запрос ~2800 токенов. Цель —
держать «горячую» часть минимальной **без потери** защит от фабрикации (уроки Sprint 2/4/5).

**Правило на каждый спринт.** Любая новая фича считает свою цену в токенах горячего
хвоста (новый инструмент = его JSON-схема в промпте на тематических вопросах).
Ориентир Sprint 2 «≤10 тулов в промпте» остаётся в силе — держится `topic_scoped_tools`.

**Рычаги (backlog, по мере надобности):**

- Нуджи `TASK_NUDGE` / `FILE_NUDGE` / `MEMORY_NUDGE` во многом дублируют разделы
  `system.md` — ужать их (они в горячем хвосте, в отличие от кэшируемого system.md).
- Держать набор инструментов на тему малым (`topic_scoped_tools`), не раздувать модули.
- Периодически мерить токены реального промпта (см. отладочный дамп `SBA_DEBUG_PROMPT`,
  если/когда добавим) и фиксировать регресс бюджета.
- Сокращать `system.md` — осторожно: он же защита доверия, строгие запреты не выкидывать.

**Метрика.** Токенов горячего хвоста на тематический запрос (нудж + схемы темы +
свежая история) — вниз или хотя бы не растёт от спринта к спринту.
...
- [ ] Sprint 10 — шаг 1 готов (2026-07-24), **ждёт живой проверки владельцем**: бэкап с автоматическим restore-тестом (`modules/backup/service.py`). **Что бэкапится**: `data/sba.db` — единственный источник истины (задачи, память, эпизоды, напоминания, аудит, тексты фрагментов документов) + конфиги `local.yaml`/`models.yaml`/`default.yaml` → zip `data/backups/sba-backup-ГГГГММДД-ЧЧММСС.zip`. Qdrant **не** бэкапится: векторный индекс полностью восстанавливается переиндексацией из SQLite (`scripts/reindex.py --full`), а embedded-Qdrant небезопасно копировать на живую. **Как**: `VACUUM INTO` — атомарный консистентный снимок работающей базы (WAL не мешает); упаковка и проверка — в worker-потоке под таймаутом (`timeout_seconds`, урок «всё внешнее обязано иметь таймаут»). **Restore-тест каждой копии сразу после создания** (риск «бэкап есть, а восстановиться нельзя»): база читается ОБРАТНО ИЗ ZIP (проверяется сам архив, а не промежуточный файл) → `PRAGMA integrity_check` + совпадение версии схемы и набора таблиц с живой базой; битый архив удаляется. **Ротация**: последние `keep_last` (14) копий. **Расписание**: ежедневный джоб через общий Scheduler (топик `backup.daily`, misfire=`deliver` — ноутбук ночью выключен, пропуск догоняет при следующем запуске; регистрация идемпотентна, как у сводки — рестарт не съедает созревшее срабатывание); Scheduler теперь создаётся при reminders ИЛИ backup. **Команда `/backup`** мимо LLM — копия по требованию + список архивов. Конфиг `modules.backup` (enabled/time/keep_last/dir/timeout_seconds). **Для владельца**: README §«Бэкап и восстановление» — пошаговая PowerShell-инструкция восстановления на чистой машине ≤ 30 мин (DoD) + совет копировать zip на флешку (бэкап на том же диске не спасает от поломки диска). Живой проверке подлежат: `/backup` в Telegram, появление zip в `data\backups`, автокопия в 14:00. **Шаг 2 готов (2026-07-24): health-мониторинг с самоотчётом (`modules/health/service.py`).** Watchdog по heartbeat: фоновые циклы (индексатор, планировщик, консолидация памяти) на каждом витке подают «сигнал жизни» через инъектированный `heartbeat`-колбэк (`run_forever(heartbeat=…)`); `HealthMonitor` периодически проверяет молчание и при превышении порога шлёт `OutgoingMessage(kind=NOTIFICATION)` во все активные каналы («⚠️ „индексатор“ завис: нет признаков работы 2 ч»), при возобновлении — «✅ снова работает». **Одно уведомление на эпизод** (флаг `alerted`), доставка терпит падение отдельного канала. Почему heartbeat, а не проба «изнутри»: молчание на витке одинаково ловит и зависший `await`, и внезапно умершую задачу. **Порог «завис» per-component** считает композиция (`app.py`) из интервала самого компонента: `max(min_silence, интервал × grace_multiplier)` — редкий индексатор (скан 2 мин) и частый планировщик (тик 15 с) не путаются, нормальная пауза цикла не ложная тревога. Команда `/health` мимо LLM — статус каждого компонента (✅ работает / ⏳ запускается / ⚠️ завис) и время последнего сигнала. Конфиг `modules.health` (enabled/check_interval_seconds/grace_multiplier/min_silence_seconds). **Отступления от плана:** (1) мониторятся именно фоновые циклы (у них есть витки для heartbeat); доступность Ollama/Groq отдельной пробой не проверяется — сбой модели и так виден владельцу в ответе, а на облаке добавлять периодический пинг = жечь лимит; (2) самоотчёт «всё хорошо» на старте не шлём (шум) — для этого `/health`. Живой проверке подлежат: `/health` в Telegram, приход уведомления при реальном зависании. Осталось по Sprint 10: ротация/retention логов и памяти, allowlist HTTP, нагрузочная проверка (NFR-5), документация эксплуатации
- ♾️ **Постоянная задача — бюджет токенов и промпта** (сквозная, не закрывается): см. секцию «Постоянная задача — бюджет токенов и промпта» в начале файла. Каждый спринт считает цену новой фичи в горячих токенах; первый кандидат на урезание — дублирование нуджей с разделами `system.md`
