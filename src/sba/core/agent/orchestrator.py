...
class AgentOrchestrator:
    ...
    def _llm_error_hint(self, exc: LLMError) -> str:
        """Подсказка под конкретный рантайм: для локальной модели — про Ollama,
        ...
        if "429" in str(exc):
            return (
                f"{head}Облачная модель упёрлась в лимит запросов в минуту "
                "(бесплатный тариф). Подождите с минуту и повторите. Если это "
                "частит — включите режим экономии токенов "
                "(agent.topic_scoped_tools: true в config/local.yaml) или "
                "выберите модель с большим лимитом."
            )
        return (
            f"{head}Проверьте интернет и API-ключ облачной модели "
            "(переменная окружения из config/models.yaml)."
        )
