...
- [x] Sprint 2 завершён (2026-07-18): Tool Registry с уровнями риска (ADR-10), agent loop с лимитом итераций, audit log, потоковый tool calling + JSON-fallback (`tool_mode: json`), инструменты `get_current_time` / `list_files` / `read_document` / `delete_file` (destructive, в корзину), файловая безопасность через `files.allowed_roots`. Отступление от плана: подтверждение destructive — текстом «да/нет» (работает в любом канале); inline-кнопки Telegram отложены к Sprint 6, где всё равно строится UX кнопок для напоминаний
