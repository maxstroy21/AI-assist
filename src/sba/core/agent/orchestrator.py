...
TASK_TOPIC_MARKERS = (
    ...
    "по проекту", "напомин", "не забыть", "не забудь", "не забывай",
...
TASK_NUDGE = (
    ...
    "закрыл задачу, без успешного вызова инструмента. Если просят регулярно "
    "что-то делать или напоминать («напомни…», «каждое утро отправляй…») — "
    "создай задачу через create_task и честно предупреди, что сам в срок не "
    "уведомишь и внешнее действие не выполнишь (это появится позже)."
)
