...
class ToolChoiceError(LLMError):
    """Модель позвала инструмент, которого не было в отправленном наборе, и
    строгий бэкенд (Groq) отклонил ход. Возникает при topic-scoped-фильтрации:
    оркестратор ловит и повторяет запрос с полным набором инструментов."""
